...
if not os.path.exists(new_image_path):
    ...
    try:
        ...
        # 3. 이미지 분류 수행
        with torch.no_grad(): # 예측 시에는 그래디언트 계산을 하지 않도록 설정 (성능 향상)
        ...
        # 모델 구조(print(model) 실행 결과)를 보고 정확한 인덱스 확인이 필요합니다.
        # 일반적으로 VGG11에서는 features[18]이 마지막 Conv2d 레이어입니다.
        target_layer = model.features[18] # 예: VGG11의 마지막 Conv2d 레이어 인덱스

        print(f"Grad-CAM 생성을 위한 타겟 레이어: {target_layer.__class__.__name__} (features[{18}])") # 인덱스도 함께 출력
        ...
        if heatmap is not None:
            ...
            plt.title(f'Grad-CAM for Predicted Class: {predicted_korean_name}') # 제목 설정
